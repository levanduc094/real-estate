...
class PagesController extends Controller
{
    ...
    public function blogshow($slug)
    {
        ...
        foreach ($post_tags as $post_tag) {
            $temps = Post_tag::where('tag_id', $post_tag->tag_id)->where('post_id','!=' , $post->id)->get();
            foreach ($temps as $temp) {
                array_push($ids, $temp->post_id);
            }
        }

        foreach ($ids as $id) {
            $temp = Post::where('id', $id)->first();
            $temp->author = User::where('id', $temp->user_id)->first()->name;
            array_push($related_news, $temp);
        }

        $post->related_news = $related_news;

        //end get related news
